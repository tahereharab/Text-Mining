...
users <- users[users$u_is_private == 0,]

text <-as.vector(gsub(","," ",users$u_media_text_norm))
...
write.csv(ldaOut.topics,file=paste("TXTLDAGibbs",k,"DocsToTopics.csv"))
   #top 10 terms in each topic
ldaOut.terms <- as.matrix(terms(ldaOut,10))
write.csv(ldaOut.terms,file=paste("TXTLDAGibbs",k,"TopicsToTerms.csv"))
...
write.csv(topicProbabilities,file=paste("TXTLDAGibbs",k,"TopicProbabilities.csv"))

#K-means
set.seed(123456789)
grpuser <- kmeans(topicProbabilities[,2:5], centers=4, nstart=10)
...
c4 <- subset(users,users$u_username %in% c4user$topicProbabilities.username.o.)

# A vector with all cluster sizes
c_percent <- c(nrow(c1), nrow(c2),nrow(c3),nrow(c4))
